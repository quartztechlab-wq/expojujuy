...
| v9 | **Inicio**: "Lo que se viene" con grilla asimétrica (tarjeta hero con foto + 2 tarjetas con cifra) y **galería fotográfica** de la ExpoJuy 2024 (carrusel con autoplay, lightbox y ruta oculta `#galeria`); `src/data/inicio.js` y `src/data/galeria.js`. |
...
- `src/data/inicio.js` — **bloque "Lo que se viene"**: los 3 destacados con su color, pill, cifra, glifo (`clip-path`) y la acción del CTA (`accion: 'agenda' | 'chat'`, que `renderVals` traduce a un handler).
- `src/data/galeria.js` — **fotos de la ExpoJuy 2024** (`src` + `label`) que alimentan el carrusel del Inicio, la ruta Galería y el lightbox. Se **importan** para que Vite las emita; en producción vienen del CMS.
...
- `assets/galeria/2024-01…06.webp` — fotos oficiales de la ExpoJuy 2024 (carrusel del Inicio, ruta Galería y lightbox); `assets/destacado-portada.webp` — portal de ingreso 2024, fondo de la tarjeta hero de "Lo que se viene".
...
11. **Galería** (ruta oculta `#galeria`, no figura en el nav ni en "Más") — Se llega desde "Explorar la galería" del Inicio. Header con bajada y conteo, **mosaico** de 3 columnas (`grid-auto-flow: dense`, filas de 240px, spans 2×2 / 2×1 cíclicos, zoom 1.04 en hover) que abre el lightbox en la foto tocada, y cierre con doble CTA (Expositores / Sobre).
12. **Footer** (global) — Línea de energía, logos oficiales sobre chips blancos, 2 columnas de navegación, redes, y la firma: "Diseñado y desarrollado por **Quartz Tech Labs**" → https://quartztechlabs.com/ con dot glow.

### Galería fotográfica del Inicio (v9)
Carrusel tipo coverflow bajo "Lo que se viene": escenario de 520px con las fotos absolutas y centradas; el desplazamiento sale de la distancia circular a la foto actual (`translate(-50% + off·62%) scale(1 / .78 / .62)`, opacidad `1 / .55 / .3`, `brightness+saturate` en las vecinas) y solo se ven dos de cada lado. La central abre el **lightbox** y muestra la pill "Ampliar"; una vecina se trae al centro. Flechas circulares de 48px alineadas al contenedor de 1240px y 6 puntos (`aria-pressed`, el activo se estira a 28px). **Autoplay** cada 4 s, detenido con el puntero encima (`_galPaused`, sin pasar por el estado), con el lightbox abierto, con la pestaña oculta, fuera del Inicio o con `prefers-reduced-motion` / `reducirAnimaciones`.

El **lightbox** es global (`role="dialog" aria-modal`): foto a `min(92vw,1400px)`, cerrar ×, flechas ‹ › y contador "N / 6 · ExpoJuy 2024"; click en el fondo cierra, Esc cierra y ← → navegan en círculo (listener en `window`, limpiado en `componentWillUnmount`). En mobile las flechas bajan a `bottom:80px` para no tapar la foto.
...
## State Management
- `route` (string), `q` + `rubro` (filtro expositores), `expoSel` (modal), `day` + `eje` + `intereses` + `saved` (agenda), `stand` + `mapQ` + `mapRubro` + `mz` (mapa: selección, buscador, filtro por rubro y zoom/paneo), `faqOpen`, `objOpen` + `vidPaused` (Sobre: desplegable de objetivos y video), `tier` + `grupo` + `tFaq` (entradas), `spSpot` (spotlight de Sponsors), `gal` + `lb` (foto centrada del carrusel y foto abierta en el lightbox, `null` = cerrado), `chatOpen` + `chatMsgs` + `chatInput`, campos + `errs` + `formOk` + `areasOpen` (contacto), `now` (countdown), `statP`/`statP2` (progreso count-up), y el bloque del portal (`user`, `ticket`, `authOpen`, `portalTab`…).
- Los datos llegan de `src/data/evento.js`, `src/data/predio.js`, `src/data/sobre.js`, `src/data/inicio.js` y `src/data/galeria.js` inyectados como `DATA` (`EXPOS`, `AGENDA`, `NOTIS`, `MAP`, `STANDS`, `FAQS`, `TIERS`, `SPONSORS`, `DESTACADOS`, `GALERIA`) → reemplazar por fetch a la API Spring Boot / CMS.

## Assets
- Fuentes Ambit y logos: **Kit de Diseño oficial del concurso** (provistos por la organización).
- Fotos incorporadas: `hero-expo.jpg` (hero), `bg-mountains.jpg` (fondo global), `destacado-portada.webp` (tarjeta hero de "Lo que se viene") y `galeria/2024-01…06.webp` (galería 2024). Los logos de sponsors y organismos son los archivos reales; KeLimpio sigue siendo un recorte de captura, pendiente del oficial.
...
- Fechas, precios, cifras, agenda, expositores y patrocinadores son datos demostrativos hasta recibir confirmación oficial. En Sobre quedan a confirmar la "17ª edición" (2024 fue la 16ª, con frecuencia bianual) y las cifras 180 / 38.000 / 9 de la edición anterior; en "Lo que se viene", los "12 países" y las "+30 startups".
